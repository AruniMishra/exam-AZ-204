...
- Filesystem storage is only storage option supported for Linux apps.
...
- to send email use SendGrid bindings in Azure Function.
...
- change feed doesn't capture deletes. Change feed also doesn't guarantee order acrosss the partition key value, order is guranteed within each logical partition key value.
...
- DeleteSnapshotsOption:  
  Required if the blob has associated snapshots. Specify one of the following two options: **include**(IncludeSnapshots): Delete the base blob and all of its snapshots. **only**(OnlySnapshots): Delete only the blob's snapshots and not the blob itself
...
- Events-lightweight notification of a condition or a state change.
...
- [validation handshake with Event Grid.](https://learn.microsoft.com/en-us/azure/event-grid/webhook-event-delivery)

- [When publishing, you must have a SAS token or key authentication for the topic.](https://learn.microsoft.com/en-us/azure/event-grid/concepts#security)

- [Topics](https://learn.microsoft.com/en-us/training/modules/azure-event-grid/2-event-grid-overview): Custom topics & Event subscriptions

## Azure Event Hub

- Azure Event Hubs is a big data streaming platform and event ingestion service. It can receive and process millions of events per second. Data sent to an event hub can be transformed and stored by using any real-time analytics provider or batching/storage adapters.

- Azure Event Hub is for telemetry and distributed data broadcasting(example temprature)

- we don't bothered about the sequencing of messages

- Capturing and Partitioning, these are features of event hub and are not available in event grid

- Number of partitions per event hub:
  - 1024 per event hub or  2000 per CU : Dedicated
  - 32 : Basic and Standard
...
- the events are not deleted (untill message retention period expires), so that multiple consumers can read the data.
...
- We can change the 'Through output' but cannot change the partition count of an event hub once it is created.
...
- To configure Manifest to include Group Claims in Auth Token
  ...
  3. Locate the "groupMembershipClaims" setting. Set its value to either "SecurityGroup" or "All". To help you decide which:
      ...
      All - groups claim will contain the identifiers of all security groups and all distribution lists/groups of which the user is a member
...
- Role-based access control (RBAC) is used for authorization and not authentication.
...
- Authenticate the app
  - You need "ID Token Claims" for authorization.  
...
- Authentication policies(API Management)
...
- If backend is accepts HTTP(S), Then Basic AUTH or Certificate will work. Basic client credentials can be defined for HTTP endpoint, not an App Service(Aure resources).

- Mutual-certificates authentication : target as 'HTTPs endpoint' + gateway credentials 'Client cert'
...
- Consumption Pricing Tier of API Management does not support Built-in Cache. Therefore caching-type should be set to "external".
...
- topics allows multiple subscribers
...
- Usage tools
  - Funnels: The progression through a series of steps in a web application is known as a funnel. You can use Application Insights Funnels to gain insights into your users, and monitor step-by-step conversion rates.
...
- URL Ping Test doesn't check for SSL Validation.

## Azure front door

- we can just make use of the **Single path purge** as the format for purging assets, that file must be between 1 KB and 8 MB in size, inclusive. No need to purge all cached assets.
...
- Azure CDN doesn't support authentication with managed identity. If you want to grant limited access to private storage containers, you can use the Shared Access Signature (SAS) feature of your Azure storage account.
...
- **authorization** level as **function**
...
- Azure Integration Service Environment for Logic Apps
  ...
  - Predicable pricing. Provides a fixed monthly cost for Logic Apps.

- PT10s means retry after 10sec. PT10S and 5 attempts means : 1st attempt plus 5 more. That is 60seconds in total.
...
- You can set the property properties.platformFaultDomainCount to 1, 2, or 3 (default of 3 if not specified).
...
- [Mount secret volume](https://docs.microsoft.com/en-us/azure/container-instances/container-instances-volume-secret)
...
- Use Microsoft Graph to read data
- Use MSAL to update the profile
...
- Set: for related items  
<https://learn.microsoft.com/en-us/azure/architecture/best-practices/caching#use-tags-to-cross-correlate-cached-items>

- List: to find the most recently accessed items  
<https://learn.microsoft.com/en-us/azure/architecture/best-practices/caching#find-recently-accessed-items>
